...
gt = cell(n_classes, length(sets)); % sets是'train'和'val'
...
for ci = 1:n_classes
    for si = 1:length(sets)
        ...
        setid = imdb.sets.(upper(sets{si}));% upper将字符串变为大写

        % get ids of images in testset
        image_ids = imdb.images.id(imdb.images.set == setid);
        % setid分别为训练集和校验集，所以image_ids为训练集或测试集的图片序号
...
if opts.concatOutput
    ...
    for ci = 1:size(gt,1)
        gt_final{ci} = cell2mat(gt(ci,:)');% VOC07 gt(ci,:)'加了转置
    end
    gt = gt_final;
end
